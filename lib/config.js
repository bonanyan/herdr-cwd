...
function loadConfig(overrides = {}) {
  ...
  const cfg = {
    ...
    poll_ms: number(raw.poll_ms, DEFAULTS.poll_ms, 100, 60000),
    idle_poll_ms: number(raw.idle_poll_ms, DEFAULTS.idle_poll_ms, 500, 600000),
    tty_refresh_ms: number(raw.tty_refresh_ms, DEFAULTS.tty_refresh_ms, 500, 600000),
    max_consecutive_errors: number(raw.max_consecutive_errors, DEFAULTS.max_consecutive_errors, 3, 100000),
  ...
  if (env.HERDR_CWD_POLL_MS) cfg.poll_ms = number(env.HERDR_CWD_POLL_MS, cfg.poll_ms, 100, 60000);
  if (env.HERDR_CWD_IDLE_POLL_MS) cfg.idle_poll_ms = number(env.HERDR_CWD_IDLE_POLL_MS, cfg.idle_poll_ms, 500, 600000);
